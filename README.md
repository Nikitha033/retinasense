...
Step 2  Download & understand ODIR-5K (images + full_df.csv)
Step 3  Data cleaning & EDA (class distribution, missing files, image sizes)
Step 4  Preprocess: extract multi-labels (N,D,G,C,A,H,M,O) + extract DR
        severity from diagnostic keyword text
...
Step 7  Model A: multi-label disease classifier
        (EfficientNet-B0 backbone, pretrained ImageNet, 8-way sigmoid head)
...
| File | Purpose |
|---|---|
| `01_data_preprocessing.py` | Loads `full_df.csv`, builds multi-labels, extracts DR severity from keywords, does patient-wise split, saves clean CSVs |
| `02_dataset.py` | PyTorch `Dataset`/`DataLoader` with augmentation pipeline |
| `03_model.py` | Model A: EfficientNet-B0 (pretrained, transfer learning) + disease/severity heads |
...
| `05_evaluate.py` | Metrics: AUC, F1, Kappa (ODIR official score), confusion matrix |
| `06_image_comparison.py` | Registration + SSIM diff for progression tracking |
| `07_inference_demo.py` | End-to-end inference on a new image / image pair |
...
- ODIR-5K gives **multi-label disease** ground truth directly (N,D,G,C,A,H,M,O)
  — that part is fully supervised.
- ODIR-5K does **not** provide a clean severity column. Severity here is
  ...
  explicitly in your report — don't claim severity grading for all 8
